...
class NeuralNet:
    ...
    def _initialize_architecture(input_size, output_size):
        ...
        architecture = nn.Sequential(
            nn.Linear(in_features=input_size, out_features=128),# Layer with 1000 nodes.
            nn.ReLU(),  # Apply ReLU activation to previous layer.
            nn.Linear(in_features=128, out_features=64),  # Layer with 750 nodes.
            nn.ReLU(),  # Apply ReLU activation to previous layer.
...
class Exp02:
    ...
    def load_train_test_data(file_path_prefix=""):
        ...
        train = np.loadtxt(file_path_prefix + "mnist_train.csv", delimiter=',', dtype=np.int)
        test = np.loadtxt(file_path_prefix + "mnist_test.csv", delimiter=',', dtype=np.int)
        x_train = train[:, 1:]
    ...
    def print_error_report(trained_model, x_train, y_train, x_test, y_test):
        ...
        mean_absolute_error_train = Exp02.compute_mean_absolute_error(y_train, y_train_pred)
        mean_absolute_perc_error_train = Exp02.compute_mean_absolute_percentage_error(y_train, y_train_pred)
        ...
        mean_absolute_error_test = Exp02.compute_mean_absolute_error(y_test, y_test_pred)
        mean_absolute_perc_error_test = Exp02.compute_mean_absolute_percentage_error(y_test, y_test_pred)
    ...
    def run(self):
        ...
        # Number of outputs == 1; Only outputing 1 variable: rings/age
        number_of_outputs = 10


        model = NeuralNet(input_size=number_of_inputs, output_size=number_of_outputs)
        #print(summary(model.architecture, input_size=(batch_size, number_of_inputs)))
